...
export async function refreshToken(malAuth) {
  const clientId = process.env.MAL_CLIENT_ID
  const clientSecret = process.env.MAL_CLIENT_SECRET
  if (!clientId || !clientSecret)
    throw Error("Need mal secret to refresh token")
...
export async function getSeasonalAnime(malAuth, year, season) {
...
export async function getAnime(malAuth, malId) {
  const config = {
    headers: {
      Authorization: `Bearer ${malAuth.accessToken}`,
    },
